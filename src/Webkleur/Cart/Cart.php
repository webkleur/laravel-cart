...
use Webkleur\Cart\Exceptions\InvalidItemException;
use Webkleur\Cart\Helpers\Helpers;
use Webkleur\Cart\Validators\CartItemValidator;
use Webkleur\Cart\Exceptions\UnknownModelException;

/**
 * Class Cart
 * @package Webkleur\Cart
 */
class Cart
{
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            $conditionInstance = "\\Webkleur\\Cart\\CartCondition";

            if ($itemCondition instanceof $conditionInstance) {
                // we need to copy first to a temporary variable to hold the conditions
                // to avoid hitting this error "Indirect modification of overloaded element of Webkleur\Cart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (!$condition instanceof CartCondition) throw new InvalidConditionException('Argument 1 must be an instance of \'Webkleur\Cart\CartCondition\'');
